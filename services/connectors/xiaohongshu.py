...
class XiaohongshuConnector(BaseConnector):
    ...
    def _get_note_detail_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "stats_selectors": [
                ".note-content .interact-info .count",
    ...
    async def _extract_note_detail(
        ...
        if use_fast_mode:
            ...
            return await self._extract_content_parallel(page, page.url, config)
        else:
            # 使用 Agent 提取
    ...
    async def login_with_cookies(self, cookies: Dict[str, str], source: str = "default",
    ...
    async def harvest_user_content(
        ...
        Args:
            user_id: 用户ID
            limit: 限制数量
            use_fast_extract: 是否使用快速提取模式

        Returns:
            笔记列表
        """
        logger.info(f"[xiaohongshu] Harvesting notes from user: {user_id}, limit={limit}, fast={use_fast_extract}")

        # 先获取用户的笔记列表
        notes_info = await self.extract_by_creator_id(user_id, limit=limit, extract_details=False)

        if not notes_info:
            logger.warning(f"[xiaohongshu] No notes found for user: {user_id}")
            return []

        if not use_fast_extract:
            # 使用原始Agent提取模式
            return notes_info
        ...
        logger.info(f"[xiaohongshu] Extracting details for {len(note_urls)} notes using fast mode")
        ...
        try:
            ...
            details = await asyncio.gather(*[extract_detail(url) for url in note_urls])
        finally:
            await browser.close()
            await p.stop()
        ...
        # 合并信息
        results = []
        for note_info, detail in zip(notes_info, details):
    ...
    async def extract_by_note_ids(
        ...
        try:
            ...
            async def extract_single(note_id: str):
                async with semaphore:
                    page = None
                    try:
                        page = await context.new_page()
                        await page.goto(f"https://www.xiaohongshu.com/explore/{note_id}", timeout=30000)
                        await page.wait_for_load_state("networkidle", timeout=10000)
                        
                        config = self._get_note_detail_config()
                        content = await self._extract_content_parallel(page, page.url, config)
            ...
            tasks = [extract_single(note_id) for note_id in note_ids]
    ...
    async def get_note_detail(
        ...
        Returns:
            笔记详情
        """
        logger.info(f"[xiaohongshu] Getting detail for note: {note_id}")
                ...
                async def extract_detail(result):
                    async with semaphore:
                        ...
                        try:
                            ...
                            config = self._get_note_detail_config()
                            content = await self._extract_content_parallel(page, result["url"], config)
